...
namespace SweetSoft.QLDA.Core.SysManager.Interfaces
{
    ...
    {
        Task LogActionAsync<T>(LogActions.Actions action, T entity, string tableName, Guid entityId, string user = "[System]", Guid? referenceId = null, string title = null, string description = null);
        Task LogChangesAsync<T>(T oldEntity, T newEntity, string tableName, Guid entityId, string user = "[System]", Guid? referenceId = null, string title = null, string description = null);
        ...
        DataTable GetRecentProjectHistory(Guid idDuAn, int numberOfRecords = 5);
    }
}
