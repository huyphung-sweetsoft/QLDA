...
(function ($) {
    ...
    var loadConfigFile = function (type) {
        ...
        var pathLoad = pluginPath + '/default.aspx/GetSetting';
        var url = '';
        //if (type == 'user') {
    ...
    var configSortField = chunks[0] || 'modified';
    var configSortOrder = chunks[1] || 'desc';
    ...
    var getAudioPlayer = function (data) {
        var url = createPreviewUrl(data['ForcePath'] + data["Path"].replace("/",""), true);
    ...
    var selectItem = function (data) {
        var url = '';
        if (data["File Type"] == "mp3") {
            url = createPreviewUrl(data['ForcePath'] + data["Path"].replace("/", ""));
            if (!url.startsWith('/Uploads') && !url.startsWith('Uploads'))
                url = '/Uploads' + url; // remove leading slash
        }
        else if (data["File Type"] == "pdf" || data["File Type"] == "glb" || data["File Type"] == "hdr" || data["File Type"] == "onnx")
        {
            url = createPreviewUrl(data['Path']);
            if (!url.startsWith('/Uploads') && !url.startsWith('Uploads'))
                url = '/Uploads' + url; // remove leading slash
        }
        else {
            const $previewUrl = data['videourl'] || (data['Preview'] || data['Thumbnail']);
            if ($previewUrl != undefined && $previewUrl != null) {
                const $arrSplit = $previewUrl.split('/').filter(Boolean); 
                $arrSplit.pop(); 
                $arrSplit.push(data["Filename"]);
                url = '/' + $arrSplit.join('/');
            }
            else
                url = createPreviewUrl(data['videourl'] || (data['Preview'] || data['Thumbnail']));
        }
        if ($.urlParam('selectFun')) {
            
            eval('parent.window.' + $.urlParam('selectFun'))($.urlParam('field_name'), url);
    ...
    var getFileInfo = function (file) {
        ...
        $.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) +
            ...
            '&time=' + new Date().getTime(), function (data) {
                ...
                var url = '';
                if (data["File Type"] == "pdf" || data["File Type"] == "glb" || data["File Type"] == "hdr" || data["File Type"] == "onnx")
                    url = '/Uploads' + createPreviewUrl(data['Path']).replace('/Uploads', '');
                else
                    url = createPreviewUrl(data['Preview'] || data['Thumbnail']);
                // Don't show copy button 
                if (data['Protected'] == 0 && !1) {
        ...
        if (file && file.length > 0) {
            ...
            $.lightbox(url, {
                ...
                onClose: function () {
                    ...
                    if (ifr.length > 0) {
                        const $url = $("#preview img").attr("src");
                        $("#preview img").attr("src", $url +"?v="+ new Date().getTime())
    ...
    $(function () {
		...
		*/
        ...
        else if (capabilities.indexOf('folder') < 0)
            $('#newfolder').remove();
